...
if __name__ == '__main__':
    ...
    plotsdir = pathlib.Path(f"experiments/toy1/plots/{privileged_policy}")
    ...
    fig.savefig(plotsdir / "reward_js_grids.pdf", bbox_inches='tight', pad_inches=0)
    plt.close(fig)
    ...
    for i, nobs in enumerate(nobss):
        ...
        ax = axes
        mean0, low0, high0 = compute_central_tendency_and_error('mean', 'sem', js_int[:, i])
        mean1, low1, high1 = compute_central_tendency_and_error('mean', 'sem', js_naiv[:, i])
        mean2, low2, high2 = compute_central_tendency_and_error('mean', 'sem', js_augm[:, i])
        ...
        # plot_mean_std(ax, nints, js_int[:, i], label="no obs", color="tab:blue")
        # plot_mean_std(ax, nints, js_naiv[:, i], label="naive", color="tab:orange")
        # plot_mean_std(ax, nints, js_augm[:, i], label="augmented", color="tab:green")

        ymax = np.nanmax([high0, high1, high2])
        ymin = np.nanmin([low0, low1, low2])

        # statistical tests
        test = "Welch t-test"  # recommended
        confidence_level = 0.05

        test_int_augm = [run_test(test, js_int[:, i, j], js_augm[:, i, j], alpha=confidence_level) for j in range(len(nints))]
        test_naiv_augm = [run_test(test, js_naiv[:, i, j], js_augm[:, i, j], alpha=confidence_level) for j in range(len(nints))]
        ...
        fig.savefig(plotsdir / f"js_nobs_{nobs}.pdf", bbox_inches='tight', pad_inches=0)
        plt.close(fig)
        ...
        ax = axes
        mean0, low0, high0 = compute_central_tendency_and_error('mean', 'sem', r_int[:, i])
        mean1, low1, high1 = compute_central_tendency_and_error('mean', 'sem', r_naiv[:, i])
        mean2, low2, high2 = compute_central_tendency_and_error('mean', 'sem', r_augm[:, i])
        plot_mean_lowhigh(ax, nints, mean0, low0, high0, label="no obs", color="tab:blue")
        plot_mean_lowhigh(ax, nints, mean1, low1, high1, label="naive", color="tab:orange")
        plot_mean_lowhigh(ax, nints, mean2, low2, high2, label="augmented", color="tab:green")
        # plot_mean_std(ax, nints, r_int[:, i], label="no obs", color="tab:blue")
        # plot_mean_std(ax, nints, r_naiv[:, i], label="naive", color="tab:orange")
        # plot_mean_std(ax, nints, r_augm[:, i], label="augmented", color="tab:green")
        ...
        fig.savefig(plotsdir / f"reward_nobs_{nobs}.pdf", bbox_inches='tight', pad_inches=0)
        plt.close(fig)
